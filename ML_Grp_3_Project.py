# -*- coding: utf-8 -*-
"""
Created on Fri May 20 18:37:35 2022
...
print("Done ...")
print("Eduacation is not major factor to have heart disease")
...
# change as required
print('\n*** Handle Outliers ***')
df = utils.HandleOutliers(df, [clsVars,"prevalentStroke",'diabetes'])
print("Done ...")
...
df = utils.HandleNullsWithMinOfMM(df, clsVars)
print("Done ...")
...
# lDropCols = utils.DropFeaturesRank(df, clsVars, 6)
#print(lDropCols)
# if lDropCols != []:
#     df = df.drop(lDropCols, axis=1)
print("Done ...")
...
df =  df.drop(['prevalentStroke','BPMeds','diabetes','currentSmoker','prevalentHyp','male','cigsPerDay'],axis=1)
...
fi = df.drop(clsVars,axis=1)
feature_importance = pd.DataFrame({'importance': model.feature_importances_}, index= fi.columns).sort_values('importance')
feature_importance
